...
from app.arbitrage import ArbitrageCalculator, TwoWayArbitrageCalculator, ThreeWayArbitrageCalculator
...
class Test_TwoWayArbitrageCalculator:
    ...
    def test_create_data_source_return_dataframe(self, event_mock, expected_dataframe):
    ...
    def test_create_combinations_return_right_number_of_combinations(self, event_mock):
        calculator = TwoWayArbitrageCalculator(event_mock)
...
class Test_ThreeWayArbitrageCalculator:
    ...
    def test_calculate_implied_probability(self, event_mock):
        ...
        result = calculator.calculate_implied_probability(home_odds=2.9,draw_odds=3.4,away_odds=2.22)
        assert result == 1.22

    def test_calculate_odds_value(self, event_mock):
        calculator = ThreeWayArbitrageCalculator(event_mock)

        result = calculator.calculate_odds_value(money_ratio=1.11, home_win=3.1, draw=4.4, away_win=3.6)
        assert result == (39.13, 27.17, 33.70)
    ...
    def test_calculate_final_return(self, event_mock):
        ...
        draw_money = 28.19
        away_money = 31.80
        result = calculator.calculate_final_return(home_return, home_money, draw_money, away_money)
        assert result == 24.03

    def test_calculate_arbitrage_return_none_if_event_not_have_chanse(self,event_mock):
        calculator = ThreeWayArbitrageCalculator(event_mock)
        result = calculator.calculate_arbitrage()
        assert result == None

    def test_calculate_arbitrage_return_dict_if_event_have_chanse(self,positive_event_mock):
        calculator = ThreeWayArbitrageCalculator(positive_event_mock)
        result = calculator.calculate_arbitrage()
        for res in result:
            print(res)
        assert result == {'STS_home_team_win': 3.5,
                'BETCLIC_draw': 4.4,
